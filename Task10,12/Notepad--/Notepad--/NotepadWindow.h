...
private:
	...
	static const int helperProcessNumber = 4;

	std::array<DWORD, helperProcessNumber> helperProcessIds;

    void saveText(); // Сохранение текста
	bool openFile(wchar_t* fileName); // Открытие файла
	void getTextFromFile();
	int getTextLength(); // Получени размера текста
	wchar_t* getText(); // Получение текста
	void filterText(); // Удалить слова не из словаря
	void createHelpers(); // Создать фильтрующие текст потоки
