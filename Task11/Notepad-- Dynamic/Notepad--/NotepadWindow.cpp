...
void CNotepadWindow::OnCommandMenu(WPARAM wParam)
{
    switch LOWORD(wParam) {
		...
		{
			typedef int (*WordCounterFunction)(wchar_t*);
			HINSTANCE lib = LoadLibrary(L"WordCounter.dll");
			WordCounterFunction WordsCount = reinterpret_cast<WordCounterFunction>(GetProcAddress(lib, "WordsCount"));
			wchar_t *text = getText();
			int count = WordsCount(text);
			wchar_t stringedWordsCount[20];
